...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  //LOG(INFO) << "pointer check";
  //LOG(INFO) << top[0] << " " << this->prefetch_data_;
  //LOG(INFO) << top[2] << " " << this->prefetch_data_dim_;
...
void ImageSegDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    int img_row, img_col;
    //LOG(INFO)<<"img:"<<lines_[lines_id_].first;
    cv_img_seg.push_back(ReadImageToCVMatDeepLab(root_folder + lines_[lines_id_].first, new_height, new_width, is_color, &img_row, &img_col));
    ...
    this->data_transformer_->TransformImageAndSeg(cv_img_seg, &(this->transformed_data_), &(this->transformed_label_), ignore_label);
    ...
    //LOG(INFO) << "height = " << top_data_dim[top_data_dim_offset];
    //LOG(INFO) << "width = " << top_data_dim[top_data_dim_offset + 1];
  }
  batch_timer.Stop();
